...
import { useWorkflowValidation, prepareWorkflowPayload, getDiagramData } from './hooks/useWorkflowValidation';
...
export default function CreateWorkflowPage() {
  ...
  // Form state
  const [workflowMetadata, setWorkflowMetadata] = useState(DEFAULT_WORKFLOW_METADATA);
  ...
  const { validationErrors } = useWorkflowValidation(
    workflowMetadata, simpleNodes, simpleEdges, nodes, edges, editorMode
  );
  ...
  const handleCreateWorkflow = useCallback(async () => {
    ...
    try {
      ...
      alert(createError || 'Failed to create workflow. Please check the console for details.');
    }
  }, [validationErrors, workflowMetadata, simpleNodes, simpleEdges, nodes, edges, editorMode, createWorkflow, createError]);
        ...
        <div className={styles.simpleMode}>
          <div className={styles.simpleLayout}>
            ...
            <main className={styles.centerPanel}>
              ...
              <div className={styles.viewModeToggle}>
                <button
                  ...
                  className={`${styles.viewModeBtn} ${centerViewMode === 'edit' ? styles.viewModeBtnActive : ''}`}
                >
                  <Edit3 size={16} /> Edit Steps
                </button>
                <button
                  onClick={handleSwitchToFlowView}
                  className={`${styles.viewModeBtn} ${centerViewMode === 'flow' ? styles.viewModeBtnActive : ''}`}
                >
            ...
            <aside className={styles.rightSidebar}>
              ...
              <SLAPanel 
                responseSLA={{
                  hours: Math.floor((workflowMetadata.response_time_sla || 0) / 60),
                  minutes: (workflowMetadata.response_time_sla || 0) % 60
                }}
                resolutionSLA={{
                  hours: Math.floor((workflowMetadata.resolution_time_sla || 0) / 60),
                  minutes: (workflowMetadata.resolution_time_sla || 0) % 60
                }}
                onResponseSLAChange={(sla) => setWorkflowMetadata(prev => ({
                  ...prev,
                  response_time_sla: (sla.hours * 60) + sla.minutes
                }))}
                onResolutionSLAChange={(sla) => setWorkflowMetadata(prev => ({
                  ...prev,

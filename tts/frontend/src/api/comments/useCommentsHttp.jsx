...
export const useCommentsHttp = (ticketId) => {
    ...
    async (page = 1) => {
      ...
      try {
        const response = await api.get(
          `${MESSAGING_API}/comments/?ticket_id=${ticketId}`
        );
        ...
        return {
          ...
          pagination: {
            ...
            total_pages: Math.ceil(
              (response.data.count || comments.length) / 10
            ),
          },
        };
      } catch (err) {
  ...
  const addReply = useCallback(
    async (parentCommentId, content, files = []) => {
      if (!ticketId) {
      ...
      try {
        ...
        const response = await api.post(
          `${MESSAGING_API}/comments/${parentCommentId}/reply/`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
  ...
  const addReaction = useCallback(async (commentId, reactionType) => {
    try {
      const response = await api.post(
        `${MESSAGING_API}/comments/${commentId}/rate/`,
        {
  ...
  const attachDocument = useCallback(async (commentId, files) => {
    ...
    try {
      ...
      const response = await api.post(
        `${MESSAGING_API}/comments/${commentId}/attach_document/`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const downloadDocument = useCallback(async (documentId, filename) => {
    try {
      const response = await api.get(
        `${MESSAGING_API}/comments/download-document/${documentId}/`,
        {
          responseType: "blob",
        }
      );
